...
export default function ShipmentDashboard() {
    ...
    const getTotalItems = (shipment: any) => {
        if (!shipment || !shipment.boxes || !Array.isArray(shipment.boxes)) return 0;

        return shipment.boxes.reduce((total: any, box: any) => {
            if (!box || !box.items || !Array.isArray(box.items)) return total;
            return total + box.items.length;
        }, 0);
    };
                ...
                <div className="bg-white rounded-lg shadow-lg max-w-3xl mx-auto overflow-hidden">
                    ...
                    <div className="p-6">
                        ...
                        {reportData?.summary && (
                            <div className="bg-gray-50 p-4 rounded-lg mb-6">
                                ...
                                <div className="grid grid-cols-3 gap-4 text-center">
                                    <div>
                                        <div className="text-2xl font-bold text-gray-800">{reportData.summary.totalItems || 0}</div>
                                        <div className="text-sm text-gray-600">Total Items</div>
                                    </div>
                                    <div>
                                        <div className="text-2xl font-bold text-green-500">{reportData.summary.approvedItems || 0}</div>
                                        <div className="text-sm text-gray-600">Approved</div>
                                    </div>
                                    <div>
                                        <div className="text-2xl font-bold text-red-500">{reportData.summary.rejectedItems || 0}</div>
                        ...
                        <div className="space-y-4 mb-6">
                            {reportData?.report && Array.isArray(reportData.report) && reportData.report.map((item: any, index: any) => (
                                <div key={index} className="border border-gray-200 rounded-lg p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <h4 className="font-semibold text-gray-800">{item?.itemName || 'Unknown Item'}</h4>
                                            <p className="text-sm text-gray-600">Manufacturer: {item?.itemManufacturer || 'N/A'}</p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm mt-2">
                                        <div>
                                            <span className="text-gray-500">Material:</span> {item?.material || 'N/A'}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Weight:</span> {item?.itemWeight ? `${item.itemWeight} kg` : 'N/A'}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">HS Code:</span> {item?.hsCode || 'N/A'}
                                        </div>
                                        <div>
                                            <span className="text-gray-500">Export Status:</span>
                                            {item?.exportStatus !== undefined ? (
                                                item.exportStatus ? (
                                                ...
                                                <span className="text-gray-500 ml-1">Unknown</span>
                                            )}
                                        </div>
                                    </div>
                                    {item?.exportStatus === false && item?.exportReason && (
                ...
                {shipments && Array.isArray(shipments) && shipments.length > 0 ? (
                    <div className="grid grid-cols-1 gap-6">
                        {shipments.map((shipment: any) => (
                            <div key={shipment?._id || `shipment-${Math.random()}`} className="bg-white rounded-lg shadow-md overflow-hidden transition-all hover:shadow-lg">
                                <div className="p-6">
                                    <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4">
                                        <div>
                                            <h2 className="text-xl font-semibold text-gray-800">{shipment?.organizationName || 'Unknown Organization'}</h2>
                                            {shipment?._id && (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                            ...
                                            {shipment?.sourceAddress && (
                                                <>
                                                    <p className="text-gray-700">
                                                        {shipment.sourceAddress.city || 'N/A'}{shipment.sourceAddress.city && shipment.sourceAddress.country ? ', ' : ''}
                                                        {shipment.sourceAddress.country || ''}
                                                    </p>
                                        ...
                                        <div className="bg-gray-50 p-4 rounded-lg">
                                            ...
                                            {shipment?.destinationAddress && (
                                                <>
                                                    <p className="text-gray-700">
                                                        {shipment.destinationAddress.city || 'N/A'}{shipment.destinationAddress.city && shipment.destinationAddress.country ? ', ' : ''}
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="space-y-3">
                                            {shipment?.boxes && Array.isArray(shipment.boxes) && shipment.boxes.map((box: any, index: any) => (
                                                <div key={index} className="border border-gray-200 rounded-lg p-3">
                                                    <div className="flex justify-between items-center">
                                                        <div className="flex items-center">
                                                            <Box size={16} className="text-blue-500 mr-2" />
                                                            <span className="font-medium text-gray-800">
                                                                {box?.boxType ? (box.boxType.charAt(0).toUpperCase() + box.boxType.slice(1)) : 'Standard'} Box
                                                            </span>
                                                        </div>
                                                        <span className="text-sm text-gray-500">
                                                            {box?.items && Array.isArray(box.items) ? box.items.length : 0} items
                                                        </span>
                                                    </div>
                                                    <div className="mt-2 grid grid-cols-2 text-sm">
                                                        <div className="text-gray-600">
                                                            Max weight: {box?.maxWeight !== undefined ? `${box.maxWeight} kg` : 'N/A'}
                                                        </div>
                                                        {box?.dimensions && (
                                                            <div className="text-gray-600">
                                                                Dimensions: {box.dimensions.length || 0}×{box.dimensions.width || 0}×{box.dimensions.height || 0} cm
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
